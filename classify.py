...
steels=["590","780","980","1180"]
train=create_dataset("train",steels=steels,feature=feature,prop_dic=prop_dic,pic_size=pic_size,sam_size=sam_size)
val=create_dataset("val",steels=steels,feature=feature,prop_dic=prop_dic,pic_size=pic_size,sam_size=sam_size)
    ...
    for __ in range(3):
      ...
      for ep in range(epoch):
        ...
        if acc>max_acc:
          ...
          torch.save(model,"model/%.3f_%s_%s.pkl"%(acc,type(model).__name__,"_".join(feature)),_use_new_zipfile_serialization=False)
